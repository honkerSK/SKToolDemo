...
@implementation NSData (SKEncrypt)

static void sk_FixKeyLengths(CCAlgorithm algorithm, NSMutableData * keyData, NSMutableData * ivData)
{
...
- (NSData*)sk_encryptedWithAESUsingKey:(NSString*)key andIV:(NSData*)iv {
    return [self sk_CCCryptData:self algorithm:kCCAlgorithmAES128 operation:kCCEncrypt key:key iv:iv];
}
/**
 *  @summary  利用AES解密据
 *
...
- (NSData*)sk_decryptedWithAESUsingKey:(NSString*)key andIV:(NSData*)iv {
    return [self sk_CCCryptData:self algorithm:kCCAlgorithmAES128 operation:kCCDecrypt key:key iv:iv];
}
/**
 *  利用3DES加密数据
 *
...
- (NSData*)sk_encryptedWith3DESUsingKey:(NSString*)key andIV:(NSData*)iv {
    return [self sk_CCCryptData:self algorithm:kCCAlgorithm3DES operation:kCCEncrypt key:key iv:iv];
}
/**
 *  @summary   利用3DES解密数据
 *
...
- (NSData*)sk_decryptedWith3DESUsingKey:(NSString*)key andIV:(NSData*)iv {
    return [self sk_CCCryptData:self algorithm:kCCAlgorithm3DES operation:kCCDecrypt key:key iv:iv];
}
...
- (NSData *)sk_encryptedWithDESUsingKey:(NSString*)key andIV:(NSData*)iv{
    return [self sk_CCCryptData:self algorithm:kCCAlgorithmDES operation:kCCEncrypt key:key iv:iv];
}
/**
 *  @summary   利用DES解密数据
 *
...
- (NSData *)sk_decryptedWithDESUsingKey:(NSString*)key andIV:(NSData*)iv{
    return [self sk_CCCryptData:self algorithm:kCCAlgorithmDES operation:kCCDecrypt key:key iv:iv];
}
- (NSData *)sk_CCCryptData:(NSData *)data
    ...
    sk_FixKeyLengths(algorithm, keyData,ivData);
