...
#import "NSData+SKzlib.h"
#import <zlib.h>
#import <dlfcn.h>

static const uInt CHUNK_SIZE = 65536;

NSString *const SKZlibErrorDomain = @"se.bitba.SKZlibErrorDomain";
NSString *const SKZlibErrorInfoKey = @"zerror";

@implementation NSData (SKzlib)

static void *sk_zlibOpen()
{
    static void *libz;
...
- (NSData *)sk_dataByInflatingWithError:(NSError *__autoreleasing *)error
{
    if (![self length]) return [self copy];
    NSMutableData *outData = [NSMutableData data];
    [self sk_inflate:^(NSData *toAppend) {
...
- (NSData *)sk_dataByDeflatingWithError:(NSError *__autoreleasing *)error
{
    if (![self length]) return [self copy];
    NSMutableData *outData = [NSMutableData data];
    [self sk_deflate:^(NSData *toAppend) {
        [outData appendData:toAppend];
    }
               error:error];
    return outData;
...
- (BOOL)sk_inflate:(void (^)(NSData *))processBlock
             error:(NSError *__autoreleasing *)error
{
    void *libz = sk_zlibOpen();
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:SKZlibErrorDomain
                                                code:SKZlibErrorCodeInflationError
                                            userInfo:@{SKZlibErrorInfoKey : @(ret)}];
        return NO;
    }
...
// Adapted from http://www.zlib.net/zpipe.c
- (BOOL)sk_deflate:(void (^)(NSData *))processBlock
             error:(NSError *__autoreleasing *)error
{
    void *libz = sk_zlibOpen();
    //    int (*deflateInit2_)(z_streamp, int, int, int, int, int, const char *, int) =
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:SKZlibErrorDomain
                                                code:SKZlibErrorCodeDeflationError
                                            userInfo:@{SKZlibErrorInfoKey : @(ret)}];
        return NO;
    }
    Bytef *source = (Bytef *)[self bytes]; // yay
...
- (BOOL)sk_writeDeflatedToFile:(NSString *)path
                         error:(NSError *__autoreleasing *)error
{
    NSFileHandle *f = sk_createOrOpenFileAtPath(path, error);
    if (!f) return NO;
    BOOL success = YES;
    if ([self length]) {
        success = [self sk_deflate:
                   ^(NSData *toAppend) {
...
- (BOOL)sk_writeInflatedToFile:(NSString *)path
                         error:(NSError *__autoreleasing *)error
{
    NSFileHandle *f = sk_createOrOpenFileAtPath(path, error);
    if (!f) return NO;
...
static NSFileHandle *sk_createOrOpenFileAtPath(NSString *path, NSError *__autoreleasing *error)
{
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        ...
        if (!success) {
            if (error) *error = [NSError errorWithDomain:SKZlibErrorDomain
                                                    code:SKZlibErrorCodeCouldNotCreateFileError
                                                userInfo:nil];
            return nil;
        }
    }
