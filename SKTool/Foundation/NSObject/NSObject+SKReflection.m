...
#import "NSObject+SKReflection.h"
#import <objc/runtime.h>

@implementation NSObject (SKReflection)
...
{
    return [[self class] sk_propertyKeys];
}
+ (NSArray *)sk_propertyKeys {
...
{
    return [[self class] sk_propertiesInfo];
}
/**
 *  属性列表与属性的各种信息
 *
...
+ (NSArray *)sk_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self sk_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)sk_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] sk_propertiesInfo];
...
-(NSArray*)sk_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] sk_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] sk_decodeType:returnType];
        NSString *encodeString = [[self class] sk_decodeType:encoding];
...
-(NSDictionary *)sk_protocolList{
    return [[self class]sk_protocolList];
}
+ (NSDictionary *)sk_protocolList
{
...
+ (NSArray *)sk_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] sk_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)sk_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)sk_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString sk_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
