...
BOOL sk_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void sk_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void sk_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
{
    sk_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)sk_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    sk_method_append(self.class, klass, newMethod);
}

+ (void)sk_replaceMethod:(SEL)method fromClass:(Class)klass
{
    sk_method_replace(self.class, klass, method);
}

- (BOOL)sk_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class sk_instancesRespondToSelector:selector untilClass:stopClass];
}
...
{
    return [self.superclass sk_instancesRespondToSelector:selector untilClass:stopClass];
}
