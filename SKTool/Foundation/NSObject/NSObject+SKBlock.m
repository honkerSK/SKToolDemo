...
#import "NSObject+SKBlock.h"
#import <dispatch/dispatch.h>

static inline dispatch_time_t sk_dTimeDelay(NSTimeInterval time) {
    int64_t delta = (int64_t)(NSEC_PER_SEC * time);
    return dispatch_time(DISPATCH_TIME_NOW, delta);
}

@implementation NSObject (SKBlock)
...
+ (id)sk_performBlock:(void (^)(void))block afterDelay:(NSTimeInterval)delay {
    ...
    dispatch_after(sk_dTimeDelay(delay), dispatch_get_main_queue(), ^{  wrappingBlock(NO); });
...
+ (id)sk_performBlock:(void (^)(id arg))block withObject:(id)anObject afterDelay:(NSTimeInterval)delay {
    ...
    dispatch_after(sk_dTimeDelay(delay), dispatch_get_main_queue(), ^{  wrappingBlock(NO, anObject); });
...
+ (void)sk_cancelPreviousPerformBlock:(id)aWrappingBlockHandle {
    [self sk_cancelBlock:aWrappingBlockHandle];
}
