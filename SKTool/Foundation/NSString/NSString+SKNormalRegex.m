...
#import "NSString+SKNormalRegex.h"

@implementation NSString (SKNormalRegex)
#pragma mark - 正则相关
- (BOOL)sk_isValidateByRegex:(NSString *)regex{
...
//手机号分服务商
- (BOOL)sk_isMobileNumberClassification{
    //运营商手机号段划分
    ...
    if (([self sk_isValidateByRegex:CM])
        || ([self sk_isValidateByRegex:CU])
        || ([self sk_isValidateByRegex:CT])
        || ([self sk_isValidateByRegex:PHS]))
    {
        return YES;
    }
    else
...
- (BOOL)sk_isMobileNumber{
    ...
    BOOL ret1 = [self sk_isValidateByRegex:mobileRegex];
    return ret1;
}
...
- (BOOL)sk_isEmailAddress{
    ...
    return [self sk_isValidateByRegex:emailRegex];
}

//身份证号
- (BOOL)sk_simpleVerifyIdentityCardNum
{
    NSString *regex2 = @"^(\\d{14}|\\d{17})(\\d|[xX])$";
    return [self sk_isValidateByRegex:regex2];
}
...
- (BOOL)sk_isCarNumber{
    ...
    return [self sk_isValidateByRegex:carRegex];
}

- (BOOL)sk_isMacAddress{
    NSString * macAddRegex = @"([A-Fa-f\\d]{2}:){5}[A-Fa-f\\d]{2}";
    return  [self sk_isValidateByRegex:macAddRegex];
}

- (BOOL)sk_isValidUrl
{
    NSString *regex = @"^((http)|(https))+:[^\\s]+\\.[^\\s]*$";
    return [self sk_isValidateByRegex:regex];
}

- (BOOL)sk_isValidChinese;
{
    NSString *chineseRegex = @"^[\u4e00-\u9fa5]+$";
    return [self sk_isValidateByRegex:chineseRegex];
}

- (BOOL)sk_isValidPostalcode {
    NSString *postalRegex = @"^[0-8]\\d{5}(?!\\d)$";
    return [self sk_isValidateByRegex:postalRegex];
}

- (BOOL)sk_isValidTaxNo
{
    NSString *taxNoRegex = @"[0-9]\\d{13}([0-9]|X)$";
    return [self sk_isValidateByRegex:taxNoRegex];
}

- (BOOL)sk_isValidWithMinLenth:(NSInteger)minLenth
...
#pragma mark - 算法相关
//精确的身份证号码有效性检测
+ (BOOL)sk_accurateVerifyIDCardNumber:(NSString *)value {
