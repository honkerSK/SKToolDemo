...
static SK_HTMLEscapeMap sk_gAsciiHTMLEscapeMap[] = {
...
static SK_HTMLEscapeMap sk_gUnicodeHTMLEscapeMap[] = {
...
static int SK_EscapeMapCompare(const void *ucharVoid, const void *mapVoid) {
    const unichar *uchar = (const unichar*)ucharVoid;
    const SK_HTMLEscapeMap *map = (const SK_HTMLEscapeMap*)mapVoid;
    int val;
    if (*uchar > map->uchar) {
        val = 1;
...
@implementation NSString (SKHTML)

- (NSString *)sk_stringByEscapingHTMLUsingTable:(SK_HTMLEscapeMap*)table
    ...
    for (NSUInteger i = 0; i < length; ++i) {
        SK_HTMLEscapeMap *val = bsearch(&buffer[i], table,
                                        size / sizeof(SK_HTMLEscapeMap),
                                        sizeof(SK_HTMLEscapeMap), SK_EscapeMapCompare);
...
- (NSString *)sk_stringByEscapingForHTML {
    return [self sk_stringByEscapingHTMLUsingTable:sk_gUnicodeHTMLEscapeMap
                                            ofSize:sizeof(sk_gUnicodeHTMLEscapeMap)
                                   escapingUnicode:NO];
} // gtm_stringByEscapingHTML

- (NSString *)sk_stringByEscapingForAsciiHTML {
    return [self sk_stringByEscapingHTMLUsingTable:sk_gAsciiHTMLEscapeMap
                                            ofSize:sizeof(sk_gAsciiHTMLEscapeMap)
                                   escapingUnicode:YES];
} // gtm_stringByEscapingAsciiHTML

- (NSString *)sk_stringByUnescapingFromHTML {
    ...
    do {
        ...
        if (length > 3 && length < 11) {
            if ([escapeString characterAtIndex:1] == '#') {
                ...
                // "standard" sequences
                for (unsigned i = 0; i < sizeof(sk_gAsciiHTMLEscapeMap) / sizeof(SK_HTMLEscapeMap); ++i) {
                    if ([escapeString isEqualToString:sk_gAsciiHTMLEscapeMap[i].escapeSequence]) {
                        [finalString replaceCharactersInRange:escapeRange withString:[NSString stringWithCharacters:&sk_gAsciiHTMLEscapeMap[i].uchar length:1]];
                        break;
                    }
                }
            }
...
- (NSString *)sk_stringWithNewLinesAsBRs {
    @autoreleasepool {
        // Strange New lines:
        //    Next Line, U+0085
        //    Form Feed, U+000C
...
- (NSString *)sk_stringByRemovingNewLinesAndWhitespace {
    @autoreleasepool {
...
- (NSString *)sk_stringByLinkifyingURLs {
...
- (NSString *)sk_stringByStrippingTags {
    @autoreleasepool {
        ...
        finalString = [result sk_stringByRemovingNewLinesAndWhitespace];
...
- (NSString *)sk_stringByConvertingHTMLToPlainText {
    @autoreleasepool {
        // Character sets
        ...
        NSString *retString = [result sk_stringByUnescapingFromHTML];
